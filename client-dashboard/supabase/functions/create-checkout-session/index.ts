...
serve(async (req) => {
  ...
  try {
    ...
    if (submissionId) {
      ...
      // Just retrieve existing user if not authenticated
      let userId = user?.id || null

      if (!userId && formData.email) {
        console.log('🔍 [AUTH] User not authenticated, fetching user by email:', formData.email)
        
        // Try to get existing user by email (user should be created at Personal Info step)
        ...
        if (!listError && users) {
          ...
          if (existingUser) {
            ...
            console.log('✅ [AUTH] Found existing user (created at Personal Info step):', userId)
          } else {
            ...
            if (authError) {
              console.error('❌ [AUTH] Failed to create account (fallback):', authError)
            } else if (authData.user) {
              userId = authData.user.id
              accountCreated = true
              console.log('✅ [AUTH] Created user account (fallback):', userId)
            }
          }
        } else {
      ...
      for (const serviceId of formData.selectedServices) {
        ...
        if (service) {
          ...
          if (documentCount > 0) {
            ...
            const localizedServiceName = localizedNames[serviceNameKey] || `${service.name} (${documentCount} document${documentCount > 1 ? 's' : ''})`
    ...
    if (Object.keys(optionCounts).length > 0) {
      for (const [optionId, count] of Object.entries(optionCounts)) {
        ...
        if (option && option.additional_price) {
          ...
          const baseOptionName = localizedNames[optionNameKey] || option.name
          // Ajouter le format avec la quantité de documents
          const localizedOptionName = `${baseOptionName} (${optionCount} document${optionCount > 1 ? 's' : ''})`
    ...
    if (additionalSignatoriesCount > 0 && additionalSignatoriesCostEUR > 0) {
      ...
      const localizedSignatoriesName = `${baseSignatoriesName} (${additionalSignatoriesCount} ${additionalSignatoriesCount > 1 ? 'signatories' : 'signatory'})`
      
      const signatoriesLineItem = {
        price_data: {
          currency: stripeCurrency,
          product_data: {
            name: localizedSignatoriesName,
