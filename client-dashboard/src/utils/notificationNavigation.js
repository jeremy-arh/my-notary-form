...
export const handleNotificationNavigation = (notification, navigate, onClose) => {
  ...
  try {
    ...
    switch (actionType) {
      ...
      case 'payout_created':
        // For notaries, redirect to payouts page
        // For admins and clients, redirect to submission detail
        if (userType === 'notary') {
          navigate('/payouts');
          if (onClose) onClose();
      ...
      default:
        ...
        if (actionData.submission_id) {
          navigate(`/submission/${actionData.submission_id}`);
          if (onClose) onClose();
        } else if (actionData.notary_id && userType === 'admin') {
          // For admin, redirect to notary detail
