    ...
    <>
                    ...
                    {documents.length > 0 && (
                      <div className="ml-9 space-y-2">
                        {documents.map((doc, index) => (
                          <div key={index} className="flex flex-col sm:flex-row sm:items-start sm:justify-between p-2 bg-gray-50 rounded-lg gap-2">
                            <div className="flex items-center flex-1 min-w-0">
                              <Icon icon="heroicons:document" className="w-5 h-5 sm:w-6 sm:h-6 mr-2 text-gray-600 flex-shrink-0" />
                            ...
                            {doc.selectedOptions && doc.selectedOptions.length > 0 && (
                              <div className="flex flex-wrap gap-1 flex-shrink-0 ml-7 sm:ml-0">
                                {doc.selectedOptions.map(optionId => {
                                  const option = optionsMap[optionId];
                                  return option ? (
                                    <span key={optionId} className="inline-flex items-center px-2 py-1 rounded-full text-[10px] sm:text-xs font-medium bg-blue-100 text-blue-800 whitespace-nowrap">
                                      <Icon icon={option.icon || "heroicons:check-badge"} className="w-3 h-3 mr-1 flex-shrink-0" />
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-200 overflow-hidden">
        ...
        <div className="space-y-3">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 bg-gray-50 rounded-xl gap-1 sm:gap-2">
            <span className="text-xs sm:text-sm font-medium text-gray-600 flex-shrink-0">Date</span>
            <span className="text-xs sm:text-sm text-gray-900 sm:text-right break-words">{formatDate(formData.appointmentDate)}</span>
          </div>
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 bg-gray-50 rounded-xl gap-1 sm:gap-2">
            <span className="text-xs sm:text-sm font-medium text-gray-600 flex-shrink-0">Time</span>
            <span className="text-xs sm:text-sm text-gray-900 sm:text-right">{formatTime(formData.appointmentTime)}</span>
          </div>
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 bg-gray-50 rounded-xl gap-1 sm:gap-2">
            <span className="text-xs sm:text-sm font-medium text-gray-600 flex-shrink-0">Timezone</span>
            <span className="text-xs sm:text-sm text-gray-900 sm:text-right break-words min-w-0">{formData.timezone || 'Not specified'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-200 overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
          <div className="p-2 sm:p-3 bg-gray-50 rounded-xl overflow-hidden">
          ...
          <div className="p-2 sm:p-3 bg-gray-50 rounded-xl overflow-hidden">
            ...
            <p className="text-xs sm:text-sm font-medium text-gray-900 break-all">{formData.email}</p>
          </div>
          <div className="p-2 sm:p-3 bg-gray-50 rounded-xl overflow-hidden">
            <p className="text-xs text-gray-600 mb-1">Phone</p>
            <p className="text-xs sm:text-sm font-medium text-gray-900 break-words">{formData.phone}</p>
          </div>
          <div className="p-2 sm:p-3 bg-gray-50 rounded-xl overflow-hidden">
            <p className="text-xs text-gray-600 mb-1">Country</p>
            <p className="text-xs sm:text-sm font-medium text-gray-900 break-words">{formData.country}</p>
          </div>
          <div className="p-2 sm:p-3 bg-gray-50 rounded-xl sm:col-span-2 overflow-hidden">
          ...
          {formData.notes && (
            <div className="p-2 sm:p-3 bg-gray-50 rounded-xl sm:col-span-2 overflow-hidden">
              ...
              <p className="text-xs sm:text-sm text-gray-900 break-words">{formData.notes}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-200 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
              ...
              <>
                {formData.selectedServices.map((serviceId, index) => {
                  ...
                  return (
                    <div key={serviceId}>
                      <div
                        className={`flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-2 ${index === 0 ? 'pb-3 border-b border-gray-200' : ''}`}
                      >
                        <span className="text-xs sm:text-sm text-gray-600 flex-1 min-w-0 break-words sm:truncate sm:pr-2">
                      ...
                      {Object.keys(optionCounts).length > 0 && (
                        <div className="ml-4 mt-2 space-y-1">
                          {Object.entries(optionCounts).map(([optionId, count]) => {
                            ...
                            return (
                              <div key={optionId} className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-2">
                                <span className="text-[10px] sm:text-xs text-gray-500 italic break-words">
                                  + {option.name} ({count} document{count > 1 ? 's' : ''})
                                </span>
                                <span className="text-[10px] sm:text-xs font-semibold text-gray-700 flex-shrink-0">
            ...
            {/* Total */}
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center pt-3 border-t-2 border-gray-300 gap-1 sm:gap-2">
              <span className="text-sm sm:text-base font-bold text-gray-900 flex-shrink-0">Total Amount</span>
      ...
      {/* Confirmation Notice */}
      <div className="bg-gray-50 border-2 border-gray-200 rounded-2xl p-3 sm:p-4 lg:p-6 overflow-hidden">
        <div className="flex items-start gap-3 sm:gap-4">
          <div className="flex-shrink-0">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gray-700 rounded-full flex items-center justify-center">
              <Icon icon="heroicons:information-circle" className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
            </div>
          </div>
