  ...
  try {
    ...
    switch (actionType) {
      ...
      case 'appointment_updated':
        // Redirect to submission detail or calendar
        if (actionData.submission_id) {
          navigate(`/submission/${actionData.submission_id}`);
          if (onClose) onClose();
        } else if (actionType === 'appointment_updated') {
          // For appointment updates, could also go to calendar
          navigate('/calendar');
          if (onClose) onClose();
        }
        break;
      ...
      case 'submission_assigned':
        // Redirect to submission detail or dashboard
        if (actionData.submission_id) {
          navigate(`/submission/${actionData.submission_id}`);
          if (onClose) onClose();
        } else {
