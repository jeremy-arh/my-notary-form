...
const SignatoriesList = ({ signatories, documentKey, documentName, showPrices = true }) => {
          ...
          <div key={sigIndex} className="bg-white rounded-lg p-2 sm:p-3 border border-gray-200">
            <div className="flex items-center justify-between mb-1.5 sm:mb-2">
              <span className="text-[10px] sm:text-xs font-semibold text-gray-900">
                ...
                {showPrices && sigIndex === 0 && <span className="ml-1.5 text-[9px] sm:text-[10px] text-gray-500">(included)</span>}
                {showPrices && sigIndex > 0 && <span className="ml-1.5 text-[9px] sm:text-[10px] text-orange-600 font-medium">(+$10)</span>}
              </span>
            </div>
